<template>
  <div class="mj">
    ...
    <div class="msg">
      <img src="../images/flyer/img_police.png" alt />
      <div>
        <div
          class="name"
        >{{data.name?data.name:"[未知]"}} {{data.no?String(data.no).replace(/^8[8|7]/, ''):"[未知]"}}</div>
        <div
          class="department"
        >{{(data.department && data.department.name)?data.department.name:"[部门未知]"}}</div>
        <div class="railway_no">{{data.train ? '当前值乘：'+data.train.name : "[值乘未知]"}}</div>
        <div
          class="phone_no"
          :title="data.phoneNo"
        >{{(data.phoneNo&&!isNaN(data.phoneNo))?data.phoneNo:"[号码未知]"}}</div>
      </div>
    </div>
    <div class="btns">
